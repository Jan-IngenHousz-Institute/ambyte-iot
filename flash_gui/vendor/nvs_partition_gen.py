...
# VENDORED for the ambyte flash bundle (from esp-idf-nvs-partition-gen 5.5).
# The `cryptography` package is used ONLY by the encrypt/decrypt/keygen paths,
# which this bundle never invokes — it only runs plain `generate`. Making the
# import optional keeps the vendored generator stdlib-only (no pip, no internet)
# for our use. If an encryption subcommand is ever run without cryptography
# installed, it fails at first use with a clear NameError, not at import.
try:
    from cryptography.hazmat.backends import default_backend
    from cryptography.hazmat.primitives import hashes, hmac
    from cryptography.hazmat.primitives.ciphers import Cipher, algorithms, modes
except ImportError:  # cryptography absent — fine unless you use encrypt/keygen
