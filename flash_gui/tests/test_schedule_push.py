...
"""Streaming a Schedule script down the console instead of the board downloading it.
...
from flash_gui.ambyte_serial import (AmbyteConsole, ConsoleError,
                                     ScheduleReleaseStatus, UnsupportedConsoleCommand)
from flash_gui.procedure import ProcedureError
from flash_gui.release_fetch import ScheduleScriptRelease, ReleaseError
...
def _script(sha=SHA, size=len(BLOB)):
    return ScheduleScriptRelease(
        ...
        asset_url="https://example.test/schedule-v1.2.0/default.yaml",
        sha256=sha, size_bytes=size, campaign_id="schedule-v1.2.0")


class _PushConsole:
    """Mimics the firmware's stateless staging: size is the file's own size."""

    def __init__(self, begin_reply="schedule begin: ready"):
    ...
    def command(self, cmd, timeout=5.0):
        ...
        if cmd.startswith("schedule commit "):
            ...
            return f"{cmd}\nschedule commit queued: id={self.committed[1]}\nambyte> "
...
def _console(**kw):
    ...
    # schedule_push is an AmbyteConsole method; bind it to the fake transport.
    console.schedule_push = AmbyteConsole.schedule_push.__get__(console, AmbyteConsole)
    console._schedule_abort_quietly = AmbyteConsole._schedule_abort_quietly.__get__(
...
def test_push_streams_every_byte_and_commits():
    console = _console()
    console.schedule_push(BLOB, SHA, "schedule-v1.2.0", "1.2.0", "v1.8.1")
    assert console.staged == BLOB
    assert console.committed == [SHA, "schedule-v1.2.0", "1.2.0", "v1.8.1"]
...
def test_chunks_fit_the_usb_jtag_rx_buffer():
    console = _console()
    console.schedule_push(BLOB, SHA, "schedule-v1.2.0", "1.2.0", "v1.8.1")
    puts = [c for c in console.commands if c.startswith("schedule put ")]
...
def test_a_dropped_chunk_is_caught_and_the_staging_file_discarded():
    ...
    def lying(cmd, timeout=5.0):
        ...
        if cmd.startswith("schedule put "):
            ...
            return reply.replace(f"schedule put: {len(console.staged)} bytes",
                                 "schedule put: 1 bytes")
        return reply
...
def test_script_bytes_verifies_the_cache_on_every_read(monkeypatch, tmp_path):
    ...
    (tmp_path / f"{SHA}.yaml").write_bytes(b"tampered")
    downloads = []
...
def test_script_bytes_reuses_a_good_cache_without_network(monkeypatch, tmp_path):
    monkeypatch.setattr(release_fetch, "SCRIPTS_CACHE_DIR", tmp_path)
    (tmp_path / f"{SHA}.yaml").write_bytes(BLOB)
    monkeypatch.setattr(
    ...
    def schedule_release(self, timeout=10.0):
        if self.committed is None and self.url_installed is None:
            return ScheduleReleaseStatus("b" * 64, "0.9.0", "v1.8.1", "v1.8.1",
                                    True, True)
        return ScheduleReleaseStatus(SHA, "1.2.0", "v1.8.1", "v1.8.1", True, True)
...
def _run(monkeypatch, console, blob=BLOB):
    ...
    return procedure.install_schedule_script(
        SimpleNamespace(schedule_script=_script(), wifi_ssid="bench-ap"),
