...
class ScheduleReleaseManifestTest(unittest.TestCase):
    def test_manifest_and_command_match_asset(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            asset = (root / "dist/default.yaml").read_bytes()
            stored = json.loads((root / "dist/default.yaml.manifest.json").read_text())
            digest = hashlib.sha256(asset).hexdigest()
            self.assertEqual(stored, manifest)
            self.assertEqual(manifest["script_name"], "default")
    ...
    def test_builds_named_script_with_distinct_asset_and_campaign(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            manifest = build(
                ...
                "4.5.6",
                "example/ambyte-iot",
                "legacy_1Hz_spec.yaml",
            )

            self.assertEqual(manifest["script_name"], "legacy_1Hz_spec")
            self.assertEqual(
                manifest["script_update"]["id"],
                "schedule-v2.0.0:legacy_1Hz_spec",
            )
            self.assertTrue((root / "dist/legacy_1Hz_spec.yaml").is_file())
            self.assertTrue(
                (root / "dist/legacy_1Hz_spec.yaml.manifest.json").is_file()
            )
    ...
    def test_rejects_asset_larger_than_firmware_parser_limit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            source = Path(tmp) / "source.yaml"
            source.write_bytes(b"x" * (16 * 1024 + 1))
            with self.assertRaisesRegex(ValueError, "firmware limit"):
                build(source, Path(tmp) / "dist", "1.0.0", "1.0.0", "example/repo")
