...
{
    return sensor_a != NULL && firmware_a != NULL && sensor_b != NULL && firmware_b != NULL &&
           strcmp(sensor_a, sensor_b) == 0 && strcmp(firmware_a, firmware_b) == 0 &&
           cal_a == cal_b;
}
...
{
    if (encoded == NULL || tuple == NULL) return false;
    ...
    if (first == NULL || second == NULL || strchr(second + 1, '|') != NULL ||
        first == encoded || strlen(second + 1) != 8U) return false;
...
static bool sensor_attached(const char *sensor,
                            const char *const attached[PAYLOAD_V3_MAX_ATTACHED])
{
    for (size_t i = 0; i < PAYLOAD_V3_MAX_ATTACHED; ++i) {
        if (attached[i] != NULL && strcmp(sensor, attached[i]) == 0) return true;
    }
    return false;
}
    ...
    size_t evict_start, bool *unchanged)
{
    ...
    if (tuples == NULL || candidate == NULL || !candidate->valid) return 0U;
    size_t empty = PAYLOAD_V3_MAX_ATTACHED;
    for (size_t i = 0; i < PAYLOAD_V3_MAX_ATTACHED; ++i) {
        if (!tuples[i].valid) {
            if (empty == PAYLOAD_V3_MAX_ATTACHED) empty = i;
            continue;
        }
        if (strcmp(tuples[i].sensor_id, candidate->sensor_id) == 0) {
            if (unchanged != NULL)
                *unchanged = payload_v3_same_device_tuple(
                    tuples[i].sensor_id, tuples[i].firmware, tuples[i].cal_version,
                    candidate->sensor_id, candidate->firmware, candidate->cal_version);
            return i;
        }
    }
    if (empty != PAYLOAD_V3_MAX_ATTACHED) return empty;

    /* Four physical channels bound the live set. Prefer an historical tuple
     * no longer attached, so reconnect dedupe state for a live sensor cannot
     * be displaced by another channel's announcement. */
    for (size_t n = 0; n < PAYLOAD_V3_MAX_ATTACHED; ++n) {
        const size_t i = (evict_start + n) % PAYLOAD_V3_MAX_ATTACHED;
        if (!sensor_attached(tuples[i].sensor_id, attached_sensor_ids)) return i;
    }
    return evict_start % PAYLOAD_V3_MAX_ATTACHED;
}
