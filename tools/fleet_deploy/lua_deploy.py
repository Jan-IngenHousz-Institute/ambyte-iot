...
class ManifestError(ValueError):
    """A Lua release manifest does not satisfy the schema-1 contract."""
...
def validate_manifest(
    ...
    if version != tag.removeprefix("lua-v"):
        raise ManifestError(
...
def _download_bytes(url: str, *, max_bytes: int) -> bytes:
    request = urllib.request.Request(
        url, headers={"User-Agent": "ambyte-lua-deploy"}
    )
