...
- Terminal `script_status` and the next STATUS heartbeat report the active script/firmware combination. Only `lua stop|status` remain as legacy diagnostics; `start|exec` fail closed while T5 removes the Lua component. See [Schedule releases and authoring](docs/schedule-releases.md).
